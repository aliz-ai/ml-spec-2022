TRAIN_QUERY = """EXPORT DATA
  OPTIONS (
    uri = 'gs://aliz-ml-spec-2022/demo-1/data/taxi_data/train/taxi_*.csv',
    format = 'CSV',
    overwrite = true,
    header = true,
    field_delimiter = ',')
...
WHERE trip_start_timestamp >= '2021-01-01 00:00:00 UTC'
    AND trip_start_timestamp < '2022-01-01 00:00:00 UTC'
    AND ABS(MOD(FARM_FINGERPRINT(trip_id), 20)) != 0
);"""

EVAL_QUERY = """EXPORT DATA
  OPTIONS (
    uri = 'gs://aliz-ml-spec-2022/demo-1/data/taxi_data/eval/taxi_*.csv',
    format = 'CSV',
    overwrite = true,
    header = true,
    field_delimiter = ',')
...
TEST_QUERY = """EXPORT DATA
  OPTIONS (
    uri = 'gs://aliz-ml-spec-2022/demo-1/data/taxi_data/test/taxi_*.csv',
    format = 'CSV',
    overwrite = true,
    header = true,
    field_delimiter = ',')
AS (
  SELECT
    ...
    IFNULL(rawDistance, 0) AS rawDistance, 
FROM `aliz-ml-spec-2022-submission.demo1.Demo1_MLdataset`
WHERE trip_start_timestamp >= '2022-01-01 00:00:00 UTC'
);"""